...
function BoardContent({ board }) {
  ...
  const findColumnByCardId = (cardId) => {
    // Đoạn này cần lưu ý, nên dùng c.cards thay vì c.cardOrderIds bởi vì ở bước handleDragOver chúng ta sex làm dữ liệu
  ...
  const moveCardBetweenDifferentColumns = (
    ...
    setOrderedColumns(prevColumns => {
      ...
      // Logic tính toán "cardIdex mới" (trên hoặc dưới của overCard) lấy chuẩn ra từ code của thư viện
      let newCardIndex
      ...
      if (nextOverColumn) {
        ...
        // Đối với trường hợp dragEnd thì phải cập nhật lại chuẩn dữ liệu columnId trong card sau khi kéo card giữa 2 column khác
  ...
  const handleDragEnd = (event) => {
    ...
    const { active, over } = event
    // Cần đảm bảo nếu không tồn tịa active hoặc over (khi kéo ra khỏi phạm vi container) thì không làm gì (tránh crash trang)
    if (!active || !over) return

    // Xử lý kéo thả Cards
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.CARD) {
      //console.log('Hành động kesothar Card - Tạm thời không làm gì cả!')
      //activeDraggingCard: là cái card đang được kéo
      const { id: activeDraggingCardId, data: { current: activeDraggingCardData } } = active
      // overCard: là cái card đang tương tác trên hoặc dưới so với cái card được kéo ở trên.
      const { id: overCardId } = over
      ...
      if (activeDragItemData._id !== overColumn._id) {
        ...
        // Dùng arrayMove vì kéo card trong một cái column thì tương tự với login kéo column trong một cái board content
        const dndOrderedCards = arrayMove(oldColumnWhenDraggingCard?.cards, oldCardIndex, newCardIndex)

        setOrderedColumns(prevColumns => {
          // Clone mảng OrderedColumnsState cũ ra một cái mới để xử lý data rồi return - cập nhập tại OrderedColumnsState mới
          const nextColumns = cloneDeep(prevColumns)
    ...
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.COLUMN) {
      ...
      if (active.id !== over.id) {
        ...
        // Dùng arrayMove của thàng dnd-kit để sắp xếp lại mảng Columns ban đầu
  ...
  const customDropAnimation = {
    slideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: '0.5' } } })
  }
  ...
  return (
    <DndContext
      ...
      // Thuật toán phát hiện va chạm (nếu không có thì card với cover lớn sẽ không kéo qua Column được vì lúc
